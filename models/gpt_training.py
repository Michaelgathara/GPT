...
class CosineWarmupScheduler:
    ...
    def step(self):
        ...
        if self.current_iter < self.warmup_iters:
            ...
            progress = float(self.current_iter - self.warmup_iters) / (
...
def estimate_loss(model, data_generator_factory, eval_iters, device, batch_size):
    ...
    for k in range(eval_iters):
        try:
            ...
            if not batch_x_list:
                logger.warning(
                    "Validation data generator yielded no data for an iteration."
                )
            ...
            logger.warning(
                f"Validation data stream ended after {k} evaluation iterations."
            )
            break  # Stop evaluation if the generator is exhausted

    model.train()  # Set model back to training mode
    avg_losses = {
        split: np.mean(split_losses) if split_losses else float("nan")
...
def create_token_generator(stream, tokenizer, block_size):
    ...
    if pad_token_id is None or pad_token_id >= tokenizer.vocab_size:
        ...
        logger.warning(
            f"Tokenizer pad_token_id {tokenizer.pad_token_id} is invalid. Using {pad_token_id} as fallback."
        )

    current_vocab_size = tokenizer.vocab_size
    logger.info(
        f"Token generator starting iteration with Vocab Size: {current_vocab_size}..."
    )
    count = 0
    skipped = 0
        ...
        if example and "input_ids" in example:
            ...
            while len(buffer) >= block_size:
                ...
                if y.min() < 0 or y.max() >= current_vocab_size:
                    ...
                    logger.error(
                        f"INVALID TARGET INDICES DETECTED in generator! Min: {min_val}, Max: {max_val}, Vocab Size: {current_vocab_size}. Skipping this chunk."
                    )
                    if invalid_targets_found > 100:
                        logger.error(
                            "Stopping detailed logging of invalid targets after 100 occurrences."
                        )
    ...
    logger.info(
        f"Token generator finished. Yielded {count} sequences, skipped {skipped} examples."
    )
...
def train_single_gpu(config, vocab_size, tokenizer, train_stream, val_stream=None):
    ...
    model = TransformerModel(
        ...
        use_mla_in_blocks=config.use_mla_in_blocks,
        mla_dropout_prob=config.mla_dropout_prob,
    )
    ...
    if os.path.isfile(checkpoint_path):
        ...
        try:
            ...
            with torch.serialization.safe_globals([np.core.multiarray.scalar]):
                ...
                )  # weights_only being false here is pretty important if you want to resume training
            ...
            logger.info(
                f"Resuming from iteration {start_iter}. Best validation loss so far: {best_val_loss:.4f}"
            )
            ...
            logger.error(
                f"Failed to load checkpoint properly: {e}. Starting training from scratch."
            )
            start_iter = 0
    ...
    logger.info(
        f"Starting training loop from iteration {start_iter} up to {config.max_iters} iterations..."
    )
    model.train()
    tokens_processed = (
        0  # Note: This counter restarts, doesn't resume total token count
    )
    ...
    for iter_num in range(start_iter, config.max_iters):
            ...
            try:
                ...
                for _ in range(config.batch_size):
                    try:
                        ...
                        logger.warning(
                            f"Training data stream ended during accumulation at iter {iter_num+1}, micro-step {micro_step+1}."
                        )
                        if not batch_x_list:
                            raise StopIteration
                        else:
                ...
                if not batch_x_list:
                    logger.warning(
                        f"No data yielded for micro-batch {micro_step+1} at iter {iter_num+1}."
                    )
                ...
                logger.warning(
                    f"Training data stream ended definitively at iteration {iter_num+1}."
                )
        ...
        if micro_batch_losses:  # Log only if an optimizer step happened
            ...
            if (iter_num + 1) % 10 == 0:
                ...
                tokens_per_sec = (
                    tokens_processed / elapsed_time if elapsed_time > 0 else 0
                )
                logger.info(
                    f"Iter {iter_num+1}/{config.max_iters}: Loss {avg_effective_loss:.4f}, LR {lr:.6f}, Tokens/Sec {tokens_per_sec:.2f}, Elapsed {elapsed_time:.2f}s"
                )

        # Check if it's time to evaluate or if it's the very last iteration
        if (iter_num + 1) % config.eval_interval == 0 or (
            iter_num + 1
        ) == config.max_iters:
            if val_stream:
                loss_dict = estimate_loss(
                    model,
                    val_generator_factory,
                    config.eval_iters,
                    device,
                    config.batch_size,
                )
                ...
                if current_val_loss < best_val_loss and current_val_loss < 4:
                    ...
                    logger.info(
                        f"New best model saved with val loss: {best_val_loss:.4f} at {checkpoint_path}"
                    )
            else:
                logger.info(
                    f"Iter {iter_num+1}: No validation data provided, skipping evaluation."
                )
                if (iter_num + 1) % (
                    config.eval_interval * 5
                ) == 0:  # Save every 5 eval intervals
                    checkpoint_path = os.path.join(
                        config.checkpoint_dir, f"checkpoint_{iter_num+1}.pt"
                    )
                    checkpoint = {
                        "model_state_dict": model.state_dict(),
                        "optimizer_state_dict": optimizer.state_dict(),
        ...
        if (
            iter_num + 1
        ) == config.max_iters and micro_step < config.accumulation_steps - 1:
            logger.warning(
                f"Exiting training loop early due to end of data stream at iteration {iter_num+1}."
            )
    ...
    start_token_id = (
        ...
        else (tokenizer.bos_token_id if tokenizer.bos_token_id is not None else 0)
    )
    ...
    try:
        ...
        logger.warning(
            "Model does not have a 'generate' method. Skipping text generation."
        )
    except Exception as e:
        logger.error(
...
if __name__ == "__main__":
    ...
    if tokenizer.pad_token is None:
        ...
        logger.info(
            f"Tokenizer already has a pad_token: {tokenizer.pad_token} (ID: {tokenizer.pad_token_id})"
        )
    ...
    try:
        ...
        logger.warning(
            "No 'validation' split found in the streaming dataset. Evaluation will be skipped unless manually configured."
        )
        val_stream_raw = train_stream_raw.take(
            1000
        )  # Take first 1000 samples for validation
        train_stream_raw = train_stream_raw.skip(1000)  # Skip these samples in training
        logger.info(
            "Created pseudo-validation set by taking first 1000 samples from train stream."
        )
        ...
        logger.error(
            "Please ensure you have internet access and are logged in to Hugging Face if required (huggingface-cli login)."
        )
    ...
    tokenized_train_stream = (
        train_stream_raw.map(tokenize_function, fn_kwargs={"tokenizer": tokenizer})
        .filter(lambda x: x is not None)
        .shuffle(buffer_size=10_000, seed=config.seed)
    )
